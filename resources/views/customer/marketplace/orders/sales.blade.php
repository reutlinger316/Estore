...
<div class="customer-dashboard-page fade-up">
    <section class="dashboard-action-box">
        <div class="dashboard-action-box__header">
            <h2>My Marketplace Sales</h2>
            <p>Products you sold in the customer marketplace.</p>
        </div>
        ...
        <div class="marketplace-product-list">
            @forelse($orders as $order)
                <div class="marketplace-product-card marketplace-product-body">
                    ...
                    <p><strong>Buyer:</strong> {{ $order->buyer->name ?? 'Deleted Buyer' }}</p>
                    ...
                    <p><strong>Sold At:</strong> {{ $order->created_at->format('d M Y, h:i A') }}</p>
                </div>
            @empty
                <p>You have not sold any marketplace products yet.</p>
            @endforelse
        </div>
    </section>
</div>
