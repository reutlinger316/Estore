...
@section('content')
    ...
    <div class="container">
        ...
        <div class="list-block">
            @forelse($cartItems as $cartItem)
                @php
                    $originalUnitPrice = (float) $cartItem->item->price;
                    $discountPercent = (float) $cartItem->item->discount;
                    $discountAmount = round(($originalUnitPrice * $discountPercent) / 100, 2);
                    $discountedUnitPrice = max(round($originalUnitPrice - $discountAmount, 2), 0);
                    $subtotal = $discountedUnitPrice * $cartItem->quantity;
                    $itemsTotal += $subtotal;
                @endphp
                    ...
                    <p>
                        ...
                        @if($cartItem->item->discount > 0)
                            ...
                            <span style="color: green; font-weight: bold; margin-left: 6px;">
                                {{ number_format($discountedUnitPrice, 2) }}
                            </span>
                        @else
                            {{ number_format($cartItem->item->price, 2) }}
                        @endif
        ...
        @if($cartItems->count() > 0)
            <div class="card">
                <h3>Items Total: <span id="items_total">{{ number_format($itemsTotal, 2) }}</span></h3>
                ...
                <form method="POST" action="{{ route('customer.cart.checkout') }}" style="margin-top: 20px;">
                    ...
                    <div id="delivery_details" style="display: none; margin-top: 15px; padding: 15px; border: 1px solid #ddd; background-color: #f9f9f9; border-radius: 5px;">
                        ...
                        <div style="margin-bottom: 15px;">
                            <label style="display:block; font-weight:bold; margin-bottom:8px;">Delivery Zone:</label>

                            <label style="display:block; margin-bottom:8px;">
                                <input type="radio" name="delivery_zone" value="inside" onchange="updateDeliveryFee()">
                                Inside {{ $cart->storeFront->delivery_city }} ({{ number_format($cart->storeFront->inside_delivery_fee, 2) }})
                            </label>

                            <label style="display:block;">
                                <input type="radio" name="delivery_zone" value="outside" onchange="updateDeliveryFee()">
                                Outside {{ $cart->storeFront->delivery_city }} ({{ number_format($cart->storeFront->outside_delivery_fee, 2) }})
                            </label>
                        </div>
                        ...
                        <div style="margin-bottom: 10px;">
                            ...
                            <small style="color: #666; margin-top: 5px; display: block;">
                                You can search or drag the pin to your exact location.
                            </small>
                        </div>
                    </div>
    ...
    <script>
        ...
        const insideFee = {{ (float) ($cart->storeFront->inside_delivery_fee ?? 0) }};
        const outsideFee = {{ (float) ($cart->storeFront->outside_delivery_fee ?? 0) }};
        ...
        function updateDeliveryFee() {
            ...
            if (document.getElementById('type_delivery').checked) {
                ...
                if (selectedZone) {
                    fee = selectedZone.value === 'inside' ? insideFee : outsideFee;
                }
            }
        ...
        function toggleDelivery() {
            ...
            if (isDelivery) {
                ...
                document.querySelectorAll('input[name="delivery_zone"]').forEach(el => el.checked = false);
            }
